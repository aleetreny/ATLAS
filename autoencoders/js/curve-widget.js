...
export function initCurveWidget(data) {
  ...
  const title = g.append('text').attr('class', 'chart-annotation')
    .attr('x', w / 2).attr('y', -70).attr('text-anchor', 'middle')
  ...
  function render() {
    ...
    SERIES.forEach(([k, , c], i) => {
      ...
      legend.append('line').attr('x1', 2).attr('x2', 24).attr('y1', -18 + i * 20)
        .attr('y2', -18 + i * 20).attr('stroke', c).attr('stroke-width', 2.6)
        .attr('stroke-dasharray', k === 'linear' ? '7 4' : null);
      legend.append('text').attr('x', 30).attr('y', -14 + i * 20)
