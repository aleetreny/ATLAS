  ...
  function render() {
    ...
    if (st.view === 'perturb') {
      ...
      rows.forEach((r) => {
        ...
        layer.append('text').attr('class', 'chart-note')
           .attr('transform', `translate(${x(r.kind) + x.bandwidth() / 2},${h + 8}) rotate(-70)`)
           .attr('text-anchor', 'end').style('font-size', '9px').text(r.kind);
      });
      ...
      rows.forEach((r) => {
        ...
        layer.append('text').attr('class', 'chart-note')
          .attr('transform', `translate(${x(r.scheme) + x.bandwidth() / 2},${h + 8}) rotate(-55)`)
           .attr('text-anchor', 'end').style('font-size', '9px').text(r.scheme);
      });
    }
