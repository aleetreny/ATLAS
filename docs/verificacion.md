...
69. **En un contenedor de la nube, `main` a secas puede no ser `origin/main`.** El paso final del procedimiento (`git checkout main && git merge --ff-only <rama>`) fallo con `refusing to merge unrelated histories`: el `main` local del clon apuntaba a un historial de 154 commits que ya no existe en el remoto, mientras `origin/main` tiene otro de 50. Los dos tienen el sitio entero y se parecen al mirarlos, asi que el error no se ve venir. **La regla: comprobar contra `origin/main`, nunca contra `main` a secas.** `git merge-base --is-ancestor origin/main <rama>` dice si el merge es de verdad fast-forward, y si lo es, `git push origin <rama>:main` publica sin tocar la rama local rota. El mismo despiste al leer `git log main` hace creer que el historial publicado tiene commits que no tiene.
70. **`git branch -r` solo enseña lo que el clon se trajo, y en la nube eso son dos ramas.** El clon del contenedor llega con `main` y la rama propia, asi que el repo parece tener dos ramas cuando en origin hay veintitres. Sale con `git fetch --prune origin` y despues `git branch -r`. Importa para el reparto de acentos, que dice mirar **las ramas sin mergear**: hacerlo sobre la lista corta es mirar a un repositorio que no existe.
